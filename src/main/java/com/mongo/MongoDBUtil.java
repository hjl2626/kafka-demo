...
public enum MongoDBUtil {
    ...
    static {
        ...
        ServerAddress address1= new ServerAddress(config.getString("mongo.host1"),config.getInt("mongo.port1"));
        ServerAddress address2= new ServerAddress(config.getString("mongo.host2"),config.getInt("mongo.port2"));
        ServerAddress address3= new ServerAddress(config.getString("mongo.host3"),config.getInt("mongo.port3"));
        ...
        options.connectionsPerHost(config.getInt("mongo.connectionsPerHost"));// 连接池设置为300个连接,默认为100
        options.connectTimeout(config.getInt("mongo.connectTimeout"));// 连接超时，推荐>3000毫秒
        options.maxWaitTime(config.getInt("mongo.maxWaitTime")); //
        options.socketTimeout(config.getInt("mongo.socketTimeout"));// 套接字超时时间，0无限制
        options.threadsAllowedToBlockForConnectionMultiplier(config.getInt("mongo.threadsAllowedToBlockForConnectionMultiplier"));// 线程队列数，如果连接线程排满了队列就会抛出“Out of semaphores to get db”错误。
...
//         Document doc = new Document();
        ...
        // interests.put("ball", "ball" + i);        // doc.put("interests", interests);
//         coll.insertOne(doc);
        // }
        ...
        cursor1 = coll.find().iterator();
        while (cursor1.hasNext()) {
            org.bson.Document _doc = cursor1.next();
            logger.info(_doc.toString());
        }
        cursor1.close();
